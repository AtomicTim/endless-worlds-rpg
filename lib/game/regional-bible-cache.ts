...
export function invalidateRegionalBibleCache(sessionId?: string): void {
  ...
  // Map keys / Set members directly without --downlevelIteration.
  const keysToDrop = Array.from(cache.keys()).filter((k) => k.startsWith(prefix));
  for (const key of keysToDrop) cache.delete(key);
  const inFlightToDrop = Array.from(inFlight).filter((k) => k.startsWith(prefix));
 ...
 * region — by the time the player walks there, the bible is warm.
 *
